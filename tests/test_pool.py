...
class TestMaxPool2d(unittest.TestCase):
    def test_maxpool(self):
        ...
        torch.testing.assert_close(F.max_pool2d(x, pool.kernel_size, pool.stride, pool.padding), pool(x))
    ...
    def test_maxpool_dilation(self):
        ...
        torch.testing.assert_close(F.max_pool2d(x, pool.kernel_size, pool.stride, pool.padding, pool.dilation), pool(x))
...
class TestAdaptiveAvgPool2d(unittest.TestCase):
    def test_adaptiveavgpool(self):
        ...
        torch.testing.assert_close(F.adaptive_avg_pool2d(x, pool.output_size), pool(x))
